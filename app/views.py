...
def orders(request):
 ...
 #changed mobile to indoor
def indoor(request, data=None):
	totalitem = 0
	if request.user.is_authenticated:
		totalitem = len(Cart.objects.filter(user=request.user))
	if data==None :
			indoor_plants = Product.objects.filter(category='M')
	elif data == 'greeny' :
			indoor_plants = Product.objects.filter(category='M').filter(brand=data)
	elif data == 'monsoon':
			indoor_plants = Product.objects.filter(category='M').filter(brand=data)
	elif data == 'below':
			indoor_plants = Product.objects.filter(category='M').filter(discounted_price__lt=500)
	elif data == 'above':
			indoor_plants = Product.objects.filter(category='M').filter(discounted_price__gt=1000)
	return render(request, 'app/indoor.html', {'indoor_plants':indoor_plants, 'totalitem':totalitem})

#function to filter outdoor data
def outdoor(request, data=None):
	totalitem = 0
	if request.user.is_authenticated:
		totalitem = len(Cart.objects.filter(user=request.user))
	if data==None :
			outdoor_plants = Product.objects.filter(category='BW')
	elif data == 'greeny' :
			outdoor_plants = Product.objects.filter(category='BW').filter(brand=data)
	elif data == 'monsoon':
			outdoor_plants = Product.objects.filter(category='BW').filter(brand=data)
	elif data == 'below':
			outdoor_plants = Product.objects.filter(category='BW').filter(discounted_price__lt=500)
	elif data == 'above':
			outdoor_plants = Product.objects.filter(category='BW').filter(discounted_price__gt=1000)
	return render(request, 'app/outdoor.html', {'outdoor_plants':outdoor_plants, 'totalitem':totalitem})

#function for medicinal plants
def medicinal(request, data=None):
	totalitem = 0
	if request.user.is_authenticated:
		totalitem = len(Cart.objects.filter(user=request.user))
	if data==None :
			medicinal_plants = Product.objects.filter(category='TW')
	elif data == 'greeny' :
			medicinal_plants = Product.objects.filter(category='TW').filter(brand=data)
	elif data == 'monsoon':
			medicinal_plants = Product.objects.filter(category='TW').filter(brand=data)
	elif data == 'below':
